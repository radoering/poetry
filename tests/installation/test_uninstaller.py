...
from poetry.installation.uninstaller import compress_for_removal
...
def test_uninstall_distribution_with_symlinked_directory(tmp_path: Path) -> None:
    ...
    # all_subdirs in compress_for_removal() relevant. Without it, the symlink
    # would be returned alongside the collapsed package-dir wildcard, and
    # remove() would rmtree the package dir first and then raise
    # FileNotFoundError trying to remove the now-missing symlink.
    env = _make_env(tmp_path)
...
def test_compress_for_removal_collapses_full_directory(tmp_path: Path) -> None:
    pkg = tmp_path / "pkg"
    pkg.mkdir()
    f1 = pkg / "a.py"
    f1.touch()
    ...
    result = compress_for_removal([str(f1), str(f2)])
    assert any(r.rstrip("/\\").endswith("pkg") for r in result)


def test_compress_for_removal_keeps_partial_directory(tmp_path: Path) -> None:
    # A directory that still holds a file we are NOT removing must not be
    # collapsed into a single directory wildcard, otherwise the unrelated file
    # would be deleted along with the directory.
    pkg = tmp_path / "pkg"
    pkg.mkdir()
    removed = pkg / "a.py"
    removed.touch()
    ...
    result = compress_for_removal([str(removed)])

    # Only the explicit file is returned, never a wildcard for the directory.
    assert result == {str(removed)}
    assert not any(r.rstrip("/\\").endswith("pkg") for r in result)
...
def test_uninstall_distribution_keeps_directory_with_unrelated_file(
    ...
    # shares the package directory must survive: the directory must not be
    # collapsed and rmtree'd out from under it.
    env = _make_env(tmp_path)
